Shader "Custom/AnimatedSun"
{
    Properties
    {
        ...
        [Header(Solar Flares)]
        _FlareStrength ("Flare Height", Float) = 0.05
        _FlareScale ("Flare Frequency", Float) = 10.0
        _FlareSpeed ("Flare Speed", Float) = 2.0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes input)
            {
                ...
                // --- SOLAR FLARE VERTEX DISPLACEMENT ---
                // We create a "spiky" displacement that moves over time
                float time = _Time.y * _FlareSpeed;
                float flareNoise = fbm(input.positionOS.xyz * _FlareScale + time);
                
                // Mask the displacement to appear mostly at the rim from the camera perspective
                float3 normalWS = TransformObjectToWorldNormal(input.normalOS);
                float rimMask = pow(1.0 - saturate(dot(normalWS, viewDir)), 2.0);
                
                float displacement = flareNoise * _FlareStrength * rimMask;
                float3 displacedPos = input.positionOS.xyz + input.normalOS * displacement;
                
                output.positionHCS = TransformObjectToHClip(displacedPos);
                output.positionOS = displacedPos;
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                granules = pow(granules, 2.0); 
                
                // 3. Macro structure (Sunspots / Magnetic activity)
                float macro = fbm(pos * 2.0 + distortion * 0.5 - time * 0.5);
                macro = smoothstep(0.3, 0.7, macro);

                // 4. SOLAR FLARE / PROMINENCE NOISE (Surface Level)
                float flareTime = _Time.y * _FlareSpeed;
                float flareNoise = fbm(pos * _FlareScale - flareTime);
                float prominences = smoothstep(0.6, 0.9, flareNoise * macro);

                // Combine granules, macro, and prominences
                float heat = granules * lerp(0.3, 1.2, macro);
                heat += prominences * 2.0; // Prominences are much hotter/brighter
                ...
                finalColor = lerp(finalColor, _ColorBright.rgb, smoothstep(0.7, 1.2, heat));
                
                // White-hot spikes for prominences
                finalColor = lerp(finalColor, half3(1,1,1), smoothstep(1.5, 3.0, heat));
                
                // 6. Limb Darkening & Corona Glow
                float ndotv = saturate(dot(normalize(input.normalWS), input.viewDirWS));
                
                // Limb darkening
                float limbDarkening = pow(ndotv, 0.5); 
                finalColor *= lerp(0.5, 1.0, limbDarkening);
                
                // Corona / Plasma edge glow
                float edgeGlow = pow(1.0 - ndotv, 4.0);
                // The edge glow is influenced by flare noise to make it "spike"
                finalColor += _ColorBright.rgb * edgeGlow * (macro + flareNoise) * 3.0;
